...
<?php elseif ($etapa == 7): ?>
    ...
    <?php elseif ($etapa == 8): ?>
        ...
        <?php if(isset($edicaoSolicitacao)):?>
            <h1 class="titulo-formulario">Solicitação editada com sucesso!</h1>
        <main>
            <p class="mensagem">Você editou o a solicitação "<?php echo $nomeProjeto ?>"</p>
            <p class="mensagem">Agora seu professor poderá rever seu projeto novamente</p>
            <a href="./perfilAluno.php" class="botao-padrao">Voltar para solicitações</a>
        </main>
            <?php else:?>
    ...
    <?php elseif ($etapa == 11): ?>
        ...
        <?php if (isset($solicitacao)): ?>
            <form action="finalizaEdicaoSolicitacao.php" method="POST" class="formulario-padrao" enctype="multipart/form-data">
            <?php else: ?>
                ...
                <?php endif; ?>
                <?php if (isset($solicitacao)): ?>
                    <input type="hidden" name="id_projeto" value="<?php echo $projeto['id_solicitacao']; ?>">
                <?php else: ?>
    ...
    <div class="checkbox-formulario">
        <?php
        // Verifica se o curso está associado ao projeto
        $checked = in_array($curso, $projetoCursos) ? "checked" : "";
        ?>
        <input type="checkbox" name="cursos[]" id="<?php echo strtolower($curso) ?>"
            value="<?php echo $id_curso ?>" <?php echo $checked; ?>>
        <label for="<?php echo strtolower($curso) ?>"><?php echo $curso ?></label>
    </div>
<?php endforeach; ?>
                ...
                <div class="botoes-formulario">
                    <?php if (isset($solicitacao)): ?>
                        <button type="button"
                            onclick="window.location.href='./detalhesSolicitacao.php?projeto=<?= $projeto['id_solicitacao'] ?>'">Voltar
                            para
                            detalhes</button>
                        <button type="submit">Continuar <i class="fa-solid fa-arrow-right"></i></button>
                    <?php else: ?>
                        <button type="button"
                            onclick="window.location.href='./detalhesProjeto.php?projeto=<?= $projeto['id_projeto'] ?>'">Voltar
                            para
                            detalhes</button>
                        <button type="submit">Continuar <i class="fa-solid fa-arrow-right"></i></button>
